...
function HomePage() {
    ...
    const memoizedAddItem = useCallback((item: any) => {
        addItem(item);
        triggerCartAnimation();
        setNotificationItemName(item.menuItem.name);
        setShowNotification(true);
    }, [addItem, triggerCartAnimation]);

    const memoizedRemoveItem = useCallback((cartItemId: string) =>
        removeItem(cartItemId), [removeItem]);

    const memoizedUpdateQuantity = useCallback((cartItemId: string, quantity: number) =>
        updateQuantity(cartItemId, quantity), [updateQuantity]);
    ...
    const filterItems = useCallback((itemsArr: readonly any[]) => {
